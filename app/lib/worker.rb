# @note All class instances inheriting from ActiveRecord::base will be postfixed
#       with "_record".
require 'pry'
require 'deep_clone'
...
module Worker
  ...
  module Maintain
      ...
      if current_user.type == 'Admin'
        if @data
          if @data[:attributes][:tree]
            recur_and_clean(@data[:attributes][:tree])
            Rails.logger.debug("COURSE: ".green + "Cleaned tree param")
          end
        end
      end
      return @data
    ...
    def self.recur_and_clean(node)
      # set num required to default if root or pivot
      if ['root', 'pivot'].include? node['type']
        if !node['numChildrenRequired'] or node['numChildrenRequired'] == ""
      ...
      if ['course'].include? node['type']
        course = @current_user.school.courses.where(subject: node['subject']).where(number: node['number']).first
        if course
          node['id'] = course.id
        else
          # Invalid course subject and/or number
          node['id'] = 0
        end
      end
      ...
      if node['children']
        node['children'].each do |child|
          recur_and_clean(child)
        end
      end
    end
  end

  # @desc All utilities related to preparing the necessary data for
  #       computing the timeline.
  module Prepare
    # @desc Creates a JSON tree formatted such that students can resolve the uncertainty added by their selected majors/minors, and removed from their prior-coursework.
    class DecisionTree
      # @param req_tree_records List of records containing `tree` definitions
      #        for req_trees.
      # @param completed_course_records List of records corresponding to the
      ...
      # @desc Returns a hash containing keys for each record's id
      def hash_record_ids(records)
        hash = {}
      ...
      def dfs_clean_and_connect_sub_trees(node)
        ...
        if ['course'].include? node['type']
          ...
          if course_record
            if course_record.tree
              # create children array on node if doesn't exist
              if !node['children']
                node['children'] = []
              end
